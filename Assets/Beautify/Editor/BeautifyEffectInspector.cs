/// <summary>
/// Beautify effect inspector. Copyright 2016-2019 Kronnect
/// </summary>
using UnityEngine;
using UnityEditor;
using System;
...
namespace BeautifyEffect {
    ...
    public class BeautifyEffectInspector : Editor {
        ...
        SerializedProperty _quality, _preset, _syncWithProfile, _compareMode, _compareLineAngle, _compareLineWidth, _profile;
        SerializedProperty _sharpen, _sharpenMinDepth, _sharpenMaxDepth, _sharpenMinMaxDepthFallOff, _sharpenDepthThreshold, _sharpenRelaxation, _sharpenClamp, _sharpenMotionSensibility;
        SerializedProperty _dither, _ditherDepth, _downscale, _superSampling;
        SerializedProperty _tonemap, _brightness, _saturate, _daltonize, _tintColor, _contrast, _hardLightBlend, _hardLightIntensity;
        SerializedProperty _bloom, _bloomDebug, _bloomCullingMask, _bloomLayerMaskDownsampling, _bloomLayerZBias, _bloomIntensity, _bloomThreshold, _bloomDepthAtten, _bloomNearAtten, _bloomMaxBrightness;
        SerializedProperty _bloomAntiflicker, _bloomUltra, _bloomUltraResolution, _bloomCustomize, _bloomWeight0, _bloomWeight1, _bloomWeight2, _bloomWeight3, _bloomWeight4, _bloomWeight5;
        ...
        SerializedProperty _anamorphicFlares, _anamorphicFlaresCullingMask, _anamorphicFlaresIntensity, _anamorphicFlaresThreshold, _anamorphicFlaresSpread, _anamorphicFlaresVertical, _anamorphicFlaresTint;
        SerializedProperty _anamorphicFlaresAntiflicker, _anamorphicFlaresUltra, _anamorphicFlaresUltraResolution, _anamorphicFlaresBlur;
        ...
        SerializedProperty _sunFlaresRevealSpeed, _sunFlaresHideSpeed, _sunFlaresSunDiskSize, _sunFlaresSunRayDiffractionIntensity;
        ...
        SerializedProperty _depthOfField, _depthOfFieldTransparencySupport, _depthOfFieldExclusionLayerMask, _depthOfFieldTransparencyLayerMask;
        ...
        SerializedProperty _depthOfFieldTargetFocus, _depthOfFieldDistance, _depthOfFieldFocusSpeed, _depthOfFieldFocalLength, _depthOfFieldAperture, _depthOfFieldForegroundBlur, _depthOfFieldForegroundBlurHQ, _depthOfFieldForegroundBlurHQSpread;
        ...
        SerializedProperty _outline, _outlineColor, _outlineCustomize, _outlineSpread, _outlineStage, _outlineBlurPassCount, _outlineIntensityMultiplier;
        ...
        SerializedProperty _antialiasStrength, _antialiasDepthThreshold;
        bool profileChanges;
        ...
        public override void OnInspectorGUI() {
            ...
            if (vrOn) {
                if (_downscale.intValue != 1) _downscale.intValue = 1;
            } else {
            ...
            if (_quality.intValue != (int)BEAUTIFY_QUALITY.Basic) {
                ...
                if (isAAEnabled) {
                    EditorGUILayout.BeginHorizontal();
                    EditorGUILayout.BeginHorizontal(GUILayout.Width(wfoldout));
                    expandAASection = EditorGUILayout.Foldout(expandAASection, new GUIContent("Anti Aliasing", "Intensity of antialiasing."), sectionHeaderNormalStyle);
                    EditorGUILayout.EndHorizontal();
                    if (_quality.intValue != (int)BEAUTIFY_QUALITY.BestQuality) {
                        ...
                        EditorGUILayout.EndHorizontal();
                        if (expandAASection) {
                            EditorGUILayout.PropertyField(_antialiasDepthThreshold, new GUIContent("   Depth Threshold", "Smooth only edges with a given depth difference."));
                        }
                    }
                }
            }
            ...
            if (_quality.intValue != (int)BEAUTIFY_QUALITY.Basic) {
                ...
                if (isHardLightEnabled) {
                    ...
                    EditorGUILayout.EndHorizontal();
                    EditorGUILayout.PropertyField(_hardLightBlend, GUIContent.none);
                    EditorGUILayout.EndHorizontal();
                    if (expandHardLightSection) {
                        EditorGUILayout.PropertyField(_hardLightIntensity, new GUIContent("   Intensity", "Intensity of hard light effect."));
                    }
                }
            ...
            if (_quality.intValue != (int)BEAUTIFY_QUALITY.Basic) {
                ...
                if (testFeatureEnabled(isBloomEnabled)) {
                    ...
                    if (expandBloomSection) {
                        ...
                        if (_quality.intValue == (int)BEAUTIFY_QUALITY.BestQuality) {
                            ...
                            EditorGUILayout.PropertyField(_bloomQuickerBlur, new GUIContent("   Quicker Blur", "Save some blur passes. Improves performance a bit in exchange of a small quality reduction."));
                ...
                bool isDoFEnabled = isShaderFeatureEnabled(Beautify.SKW_DEPTH_OF_FIELD, false) && depthBasedEffects;
                if (!isDoFEnabled) {
                    if (_depthOfField.boolValue) {
                        _depthOfField.boolValue = false;
                    }
                }
                bool isDoFTransp = isShaderFeatureEnabled(Beautify.SKW_DEPTH_OF_FIELD_TRANSPARENT, false) && depthBasedEffects;
                if (!isDoFTransp) {
                ...
                if (testFeatureEnabled(isDoFEnabled)) {
                    ...
                    if (expandDoFSection) {
                        ...
                        if (testFeatureEnabled(isDoFTransp)) {
                            ...
                            if (_depthOfFieldTransparencySupport.boolValue) {
                ...
                if (testFeatureEnabled(isOutlineEnabled)) {
                    ...
                    if (expandOutlineSection) {
                        ...
                        if (_outline.boolValue) {
                            EditorGUILayout.HelpBox("Choose between depth-based or color-based edge detection algorithm in Shader Options section. Use the alpha value of the outline color to control outline threshold.", MessageType.Info);
                        }
                        if (_quality.intValue == (int)BEAUTIFY_QUALITY.BestQuality) {
