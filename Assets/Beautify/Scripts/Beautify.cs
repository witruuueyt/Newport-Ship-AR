...
namespace BeautifyEffect {
    ...
    public partial class Beautify : MonoBehaviour {
        ...
        const string BEAUTIFY_BUILD_HINT = "BeautifyBuildHint85RC3";
        ...
        void DoOnPreRenderTasks() {
            ...
            if ((_bloom && _bloomCullingMask > 0) || (_anamorphicFlares && _anamorphicFlaresCullingMask > 0)) {
        ...
        void OnPreRender() {
            ...
            if (scaleFactor != 1f && rtDescBase.width > 1) {
                ...
                if (scaleFactor <= 1) {
                    ...
                    rtPixDesc.msaaSamples = QualitySettings.antiAliasing;
                    isSuperSamplingActive = true;
                }
                rtPixDesc.width = Mathf.RoundToInt(Mathf.Clamp(currentCamera.pixelWidth / scaleFactor, 1, 8192));
                float aspectRatio = (float)currentCamera.pixelHeight / currentCamera.pixelWidth;
                rtPixDesc.height = Mathf.RoundToInt(Mathf.Clamp(rtPixDesc.width * aspectRatio, 1, 8192));
        ...
        protected virtual void OnRenderImage(RenderTexture source, RenderTexture destination) {
            ...
            if (allowExtraEffects) {
                ...
                if (_depthOfField) {
#if UNITY_EDITOR
                    if (sceneCamera == null && cam != null && cam.cameraType == CameraType.SceneView) {
                        sceneCamera = cam;
                    }
                ...
                if (_bloom || (_lensDirt && !_anamorphicFlares)) {
                    ...
                    if (_bloom) {
                        ...
                        if (customizesBloom) {
                            ...
                            bMat.SetTexture(ShaderParams.BloomTexture, rt[0]);
                ...
                if (_anamorphicFlares) {
                    ...
                    if (_quality == BEAUTIFY_QUALITY.BestPerformance) {
                        ...
                        if (_anamorphicFlaresUltra) {
                            sizeAF = (int)Mathf.Lerp(512, source.height, _anamorphicFlaresUltraResolution / 10);
                        } else {
                            sizeAF = 512;
                        }
                    }
                    ...
                    if (_anamorphicFlaresCullingMask > 0) {
                        if (_bloom) {
            ...
            if (rtBeauty != null) {
                ...
                bMat.SetTexture(ShaderParams.CompareTexture, rtBeauty);
            ...
            if (rtBlurTex != null) {
                ...
                if (rtBeauty != null) {
                    bMat.SetTexture(ShaderParams.CompareTexture, rtBlurTex);
        ...
        public void UpdateMaterialPropertiesNow() {
            ...
            // AA
            bMat.SetVector(ShaderParams.AntialiasData, new Vector3(_antialiasStrength, _antialiasDepthThreshold, 0));
            ...
            // frame FX
            if (_frame) {
                Vector4 frameColorAdjusted = new Vector4(_frameColor.r, _frameColor.g, _frameColor.b, _frameMask != null ? _frameColor.a : (1.00001f - _frameColor.a) * 0.5f);
            ...
            if (_compareMode) {
                renderPass = 0;
                bMat.SetVector(ShaderParams.CompareData, new Vector4(Mathf.Cos(_compareLineAngle), Mathf.Sin(_compareLineAngle), -Mathf.Cos(_compareLineAngle), _compareLineWidth));
            }
            ...
            if (_quality != BEAUTIFY_QUALITY.Basic) {
                ...
                if (_chromaticAberration) {
                    ...
                    if (!_depthOfField) {
                        shaderKeywords.Add(SKW_CHROMATIC_ABERRATION);
                    }
                }
