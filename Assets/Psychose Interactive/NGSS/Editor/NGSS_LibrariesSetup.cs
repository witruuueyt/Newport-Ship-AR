...
#if !UNITY_5
    bool installShadowBiasLibrary = false;
    bool shadowBiasLibrary = false;
#endif
...
#else
        minSize = new Vector2(350, 410);
        maxSize = new Vector2(350, 410);
#endif
        if (!isInit)
        {
            try
...
#if !UNITY_5
                installShadowBiasLibrary = File.Exists(Path.Combine(cgIncludesPath, "UnityStandardShadow.cginc.bak"));
                shadowBiasLibrary = installShadowBiasLibrary;
                if (Application.platform == RuntimePlatform.WindowsEditor && Directory.Exists(cgIncludesPath)) { Process.Start("explorer.exe", cgIncludesPath); }
#endif
        ...
        if (isInit)
        {
...
#if !UNITY_5
            installShadowBiasLibrary = GUI.Toggle(new Rect(10, 210, 310, 50), installShadowBiasLibrary, !installShadowBiasLibrary ? "Install NGSS Shadow Bias library" : "Uninstall NGSS Shadow Bias library", "Button");
            Rect shaderCacheButtonRect = new Rect(10, 270, 310, 50);
#else
            ...
            if (localLibrary != installLocalLibrary)
            {
                try
                {
                    ...
                    {
                        if (GUI.changed && string.IsNullOrEmpty(NGSSAssetPath) == false && (NGSSAssetPath[NGSSAssetPath.Length - 1] == '/' || NGSSAssetPath[NGSSAssetPath.Length - 1].ToString() == @"\")) { NGSSAssetPath = NGSSAssetPath.Remove(NGSSAssetPath.Length - 1); }
...
#if !UNITY_5
            if (shadowBiasLibrary != installShadowBiasLibrary)
            {
                try
                {
                    displayRestart = true;
                    shadowBiasLibrary = installShadowBiasLibrary;
                    if (installShadowBiasLibrary)//install it
                    {
                        if (GUI.changed && string.IsNullOrEmpty(NGSSAssetPath) == false && (NGSSAssetPath[NGSSAssetPath.Length - 1] == '/' || NGSSAssetPath[NGSSAssetPath.Length - 1].ToString() == @"\")) { NGSSAssetPath = NGSSAssetPath.Remove(NGSSAssetPath.Length - 1); }
                        ...
                        {
                            installer_filename = Application.dataPath + NGSSAssetPath + "/NGSS/Editor/NGSS_Installer.exe";
                            var proc = Process.Start(installer_filename, "INSTALL_SHADOWBIAS " + "\"" + Application.dataPath + NGSSAssetPath + "\"" + " " + "\"" + cgIncludesPath + "\"");
                        }
                        else
                        {
                            if (File.Exists(Application.dataPath + NGSSAssetPath + "/NGSS/Libraries/UnityStandardShadow.cginc"))
                            {
                                //File.Replace(Application.dataPath + NGSSAssetPath + "/NGSS/Libraries/UnityStandardShadow.cginc", cgIncludesPath + "/UnityStandardShadow.cginc", cgIncludesPath + "/UnityStandardShadow.cginc.bak");
                                File.Move(cgIncludesPath + "/UnityStandardShadow.cginc", cgIncludesPath + "/UnityStandardShadow.cginc.bak");//rename
                                File.Copy(Application.dataPath + NGSSAssetPath + "/NGSS/Libraries/UnityStandardShadow.cginc", cgIncludesPath + "/UnityStandardShadow.cginc", true);//copy
                            }
                            else
                            {
                                //GUI.Label(new Rect(10, 15, 330, 75), Application.dataPath + NGSSAssetPath + "/NGSS/Libraries/UnityStandardShadow.cginc does not exist make sure the file exist!");
                                installLog = Application.dataPath + NGSSAssetPath + "/NGSS/Libraries/UnityStandardShadow.cginc does not exist make sure the file exist!";
                                UnityEngine.Debug.LogWarning(installLog);
                            }
                        }

                        installLog = "NGSS Shadow Bias libraries files successfully installed. Unity Shadow Bias libraries have been backed up. Make sure your scene lights have it's corresponding NGSS components.\nPlease restart the Editor to apply changes.";
                        UnityEngine.Debug.Log(installLog);
                    }
                    else//revert it
                    {
                        ...
                        {
                            installer_filename = Application.dataPath + NGSSAssetPath + "/NGSS/Editor/NGSS_Installer.exe";
                            var proc = Process.Start(installer_filename, "DELETE_SHADOWBIAS " + "\"" + Application.dataPath + NGSSAssetPath + "\"" + " " + "\"" + cgIncludesPath + "\"");
                        }
                        else
                        {
                            string autoLightPath = Path.Combine(cgIncludesPath, "UnityStandardShadow.cginc");
                            string autoLighBaktPath = Path.Combine(cgIncludesPath, "UnityStandardShadow.cginc.bak");
                            if (File.Exists(autoLightPath))
                                File.Delete(autoLightPath);
                            if (File.Exists(autoLighBaktPath))
                                File.Move(autoLighBaktPath, autoLightPath);
                        ...
                        installLog = "Unity Shadow Bias libraries successfully restored. NGSS Shadow Bias libraries have been deleted.\nPlease restart the Editor to apply changes.";
                        UnityEngine.Debug.Log(installLog);
                    }
                }
                catch (System.Exception ex)
                {
                    UnityEngine.Debug.Log(ex.Message);
                    installLog = ex.Message;
                    installShadowBiasLibrary = !installShadowBiasLibrary;
                    shadowBiasLibrary = installShadowBiasLibrary;
                }
                finally
                {
