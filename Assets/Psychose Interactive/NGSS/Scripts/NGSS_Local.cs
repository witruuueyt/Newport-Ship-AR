...
public class NGSS_Local : MonoBehaviour
{
    ...
    public bool NGSS_SHOW_MULTIPLE_INSTANCES_WARNING = true;
    ...
    {
#if UNITY_EDITOR
        if (NGSS_SHOW_MULTIPLE_INSTANCES_WARNING)
        {
            var ngss_local_instances = FindObjectsOfType<NGSS_Local>();
            if (ngss_local_instances.Length > 1)
            {
                Debug.LogWarning("Only one instance of NGSS_Local class is allowed in the scene. The first object of type NGSS_Local will be used, the rest will be disabled.\n" +
                    "Normally you want to attach this component to the most important Local light in your scene as it will act as a manager for local shadows.\n" +
                    "The reason is to avoid having many unnecesary instances of this class in your scene.\n" +
                    "If you want to tweak softness and filter type for any other local light, you can do it by changing the Light properties ShadowsType and Shadows Strength.", this);
    ...
    void Update()
    {
        if (LocalLight.shadows == LightShadows.None || LocalLight.type == LightType.Directional)
            return;

        if (Application.isPlaying)
        {
            if (NGSS_DISABLE_ON_PLAY) { enabled = false; return; }
            if (NGSS_NO_UPDATE_ON_PLAY) { return; }
        }
